...
### 불필요한 공백 문자(Trailing whitespace) 처리

더 자세한 내용은 [기능](#features) 및 [상세 문서](../doc/mini-trailspace.qmd)를 참조하세요.
...
⦿ 이 모듈은 [mini.nvim](https://nvim-mini.org/mini.nvim) 라이브러리의 일부입니다. 이 모듈을 언급할 때는 [이 링크](https://nvim-mini.org/mini.nvim/readmes/mini-trailspace)를 사용해 주세요.
...
- 하이라이트는 기본적으로 수정 가능한 버퍼, 노멀 모드에서만 수행되며 입력 모드나 창을 벗어날 때는 중단됩니다.
- [`MiniTrailspace.trim()`](../doc/mini-trailspace.qmd#minitrailspace.trim)으로 모든 뒷공백을 제거합니다.
- [`MiniTrailspace.trim_last_lines()`](../doc/mini-trailspace.qmd#minitrailspace.trim_last_lines)로 마지막에 위치한 모든 빈 줄을 제거합니다.
...
**중요**: 기능을 활성화하려면 `require('mini.trailspace').setup()`을 호출해야 합니다.
...
{
  -- 일반 버퍼('buftype'이 비어 있는 버퍼)에서만 하이라이트합니다. 
  -- 이는 일반적으로 공백이 중요하지 않은 곳에 하이라이트를 표시하지 않는 데 유용합니다.
