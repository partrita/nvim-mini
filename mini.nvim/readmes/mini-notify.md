...
더 자세한 내용은 [기능](#features) 및 [상세 문서](../doc/mini-notify.qmd)를 참조하세요.
...
⦿ 이 모듈은 [mini.nvim](https://nvim-mini.org/mini.nvim) 라이브러리의 일부입니다. 이 모듈을 언급할 때는 [이 링크](https://nvim-mini.org/mini.nvim/readmes/mini-notify)를 사용해 주세요.
...
- 하나 이상의 하이라이트된 알림을 단일 플로팅 창에 표시합니다.

- 알림을 관리합니다 (추가, 업데이트, 제거, 지우기).

- 사용자 정의 `vim.notify()` 구현을 제공합니다. 이를 조정하려면 `setup()`을 호출한 후 [`MiniNotify.make_notify()`](../doc/mini-notify.qmd#mininotify.make_notify)를 사용하세요.

- LSP 진행 상황 자동 보고 기능을 제공합니다.

- 알림 기록(history)을 추적하며, [`MiniNotify.get_all()`](../doc/mini-notify.qmd#mininotify.get_all)로 확인하거나 [`MiniNotify.show_history()`](../doc/mini-notify.qmd#mininotify.show_history)로 표시할 수 있습니다.
...
**중요**: 기능을 활성화하려면 `require('mini.notify').setup()`을 호출해야 합니다.
...
-- `setup()` 안에 이 내용을 복사할 필요는 없습니다. 자동으로 사용됩니다.
{
  ...
  content = {
    -- 알림 메시지를 형식화하는 함수
    -- 기본적으로 메시지 앞에 알림 시간을 붙입니다.
    format = nil,

    -- 알림 배열을 중요도 순으로 정렬하는 함수
    -- 기본적으로 레벨(level) 순으로 먼저 정렬한 후 업데이트 타임스탬프 순으로 정렬합니다.
    sort = nil,
  },
  ...
  window = {
    ...
    -- 가용한 열 너비의 비율(0에서 1 사이)로 표시되는 최대 창 너비
