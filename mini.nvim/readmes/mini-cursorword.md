...
더 자세한 내용은 [기능](#features) 및 [상세 문서](../doc/mini-cursorword.qmd)를 참조하세요.
...
⦿ 이 모듈은 [mini.nvim](https://nvim-mini.org/mini.nvim) 라이브러리의 일부입니다. 이 모듈을 언급할 때는 [이 링크](https://nvim-mini.org/mini.nvim/readmes/mini-cursorword)를 사용해 주세요.
...
- 현재 커서 아래에 있는 단어를 다르게 하이라이트할 수 있습니다.
- "커서 아래의 단어"는 Vim의 `<cword>`와 동일합니다: 사용자가 'iw' 텍스트 오브젝트로 얻게 되는 것을 의미합니다.
- 하이라이트는 Insert 및 Terminal 모드에서는 중단됩니다.
...
**중요**: 기능을 활성화하려면 `require('mini.cursorword').setup()`을 호출해야 합니다.
...
{
  -- 커서 이동 후 하이라이트가 나타날 때까지의 지연 시간 (ms)
  delay = 100,
}
```
