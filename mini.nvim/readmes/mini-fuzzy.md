...
### 가볍고 빠른 퍼지 매칭(fuzzy matching)

더 자세한 내용은 [기능](#features) 및 [상세 문서](../doc/mini-fuzzy.qmd)를 참조하세요.
...
⦿ 이 모듈은 [mini.nvim](https://nvim-mini.org/mini.nvim) 라이브러리의 일부입니다. 이 모듈을 언급할 때는 [이 링크](https://nvim-mini.org/mini.nvim/readmes/mini-fuzzy)를 사용해 주세요.
...
- 하나의 문자열을 다른 문자열들과 퍼지 매칭하는 함수를 제공합니다.
- [nvim-telescope/telescope.nvim](https://github.com/nvim-telescope/telescope.nvim)용 소터(sorter)를 제공합니다.
...
**중요**: `require('mini.fuzzy').setup()`을 반드시 호출할 필요는 없으나, 사용 편의성을 높이기 위해 호출할 수 있습니다.
...
{
  -- 일치하는 특징(너비 및 첫 번째 일치 위치)의 최대 허용값.
  -- 컷오프(cutoff)보다 큰 모든 특징 값은 "동일하게 좋지 않은 것"으로 간주됩니다.
  cutoff = 100,
}
```
