...
### 창 레이아웃을 유지하는 버퍼 제거 (안 보이기, 삭제, 완전 삭제)

더 자세한 내용은 [기능](#features) 및 [상세 문서](../doc/mini-bufremove.qmd)를 참조하세요.
...
⦿ 이 모듈은 [mini.nvim](https://nvim-mini.org/mini.nvim) 라이브러리의 일부입니다. 이 모듈을 언급할 때는 [이 링크](https://nvim-mini.org/mini.nvim/readmes/mini-bufremove)를 사용해 주세요.
...
현재 버퍼가 제거된 후 창에 어떤 버퍼를 표시할지는 다음 알고리즘에 의해 결정됩니다:

- 대체(alternate) 버퍼가 목록에 있으면 그것을 사용합니다.
- 이전 목록(listed) 버퍼가 현재와 다르면 그것을 사용합니다.
- 그렇지 않으면 `nvim_create_buf(true, true)`를 사용하여 임시(scratch) 버퍼를 생성하고 사용합니다.
...
**중요**: `require('mini.bufremove').setup()`을 호출할 필요는 없지만, 사용 편의성을 높이기 위해 호출할 수 있습니다.
